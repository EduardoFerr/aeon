/* empty css             */(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))i(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function t(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(r){if(r.ep)return;r.ep=!0;const s=t(r);fetch(r.href,s)}})();function h(n){const e=document.getElementById("app");e.innerHTML="",n==="all-cards"?e.innerHTML="<all-cards-screen></all-cards-screen>":n==="three-card-game"?e.innerHTML="<three-card-game-screen></three-card-game-screen>":n==="history"?e.innerHTML="<history-screen></history-screen>":e.innerHTML="<home-screen></home-screen>"}function v(){document.querySelectorAll("nav a").forEach(e=>{e.addEventListener("click",t=>{const i=e.getAttribute("href");if(i&&i.endsWith(".html"))return;t.preventDefault();const r=e.getAttribute("data-page");console.log("Navegando para:",r),h(r)})})}document.addEventListener("DOMContentLoaded",()=>{v(),h("home")});class C extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `,document.querySelector("header>h1").textContent="Tarot de Thoth"}}customElements.define("home-screen",C);const b="modulepreload",S=function(n){return"/"+n},p={},T=function(e,t,i){let r=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),a=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));r=Promise.allSettled(t.map(c=>{if(c=S(c),c in p)return;p[c]=!0;const l=c.endsWith(".css"),u=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${u}`))return;const d=document.createElement("link");if(d.rel=l?"stylesheet":b,l||(d.as="script"),d.crossOrigin="",d.href=c,a&&d.setAttribute("nonce",a),document.head.appendChild(d),l)return new Promise((f,y)=>{d.addEventListener("load",f),d.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${c}`)))})}))}function s(o){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=o,window.dispatchEvent(a),!a.defaultPrevented)throw o}return r.then(o=>{for(const a of o||[])a.status==="rejected"&&s(a.reason);return e().catch(s)})};let m=null;async function g(){if(m)return m;try{const e=await(await fetch("assets/data/cards.json")).json();return m=e,e}catch(n){throw console.error("Erro ao carregar as cartas:",n),n}}T(()=>import("./TarotCard-BhMfcB2C.js"),[]);class E extends HTMLElement{async connectedCallback(){this.innerHTML=`
            <div class="all-cards-screen">
                <div class="card-container" id="all-cards"></div>
                <button onclick="navigateTo('home')">Voltar</button>
            </div>
        `,document.querySelector("header>h1").textContent="Todas as Cartas",await this.renderCards()}async renderCards(){try{const e=await g();e.sort((i,r)=>i.order-r.order);const t=this.querySelector("#all-cards");t.innerHTML=e.map(i=>`
        ...
        `,document.querySelector("header>h1").textContent="Jogo das 3 Cartas",document.getElementById("drawCards").addEventListener("click",this.drawThreeCards.bind(this))}async drawThreeCards(){try{const t=(await g()).sort(()=>.5-Math.random()).slice(0,3),i=this.querySelector("#cards"),r=["Passado","Presente","Futuro"];i.innerHTML=t.map((s,o)=>`
            ...
            <div class="history-screen">
                <ul>
                    ${e.map(t=>{const i=Object.values(t.cards);return`
                                <li>${t.date}: ${i.map(r=>r.name).join(", ")}</li>
                            `}).join("")}
                </ul>
                <button onclick="navigateTo('home')">Voltar</button>
            </div>
