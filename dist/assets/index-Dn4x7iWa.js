(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&a(n)}).observe(document,{childList:!0,subtree:!0});function t(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function a(r){if(r.ep)return;r.ep=!0;const s=t(r);fetch(r.href,s)}})();function h(o){const e=document.getElementById("app");e.innerHTML="",o==="all-cards"?e.innerHTML="<all-cards-screen></all-cards-screen>":o==="three-card-game"?e.innerHTML="<three-card-game-screen></three-card-game-screen>":o==="history"?e.innerHTML="<history-screen></history-screen>":e.innerHTML="<home-screen></home-screen>"}function v(){document.querySelectorAll("nav a").forEach(e=>{e.addEventListener("click",t=>{const a=e.getAttribute("href");if(a&&a.endsWith(".html"))return;t.preventDefault();const r=e.getAttribute("data-page");console.log("Navegando para:",r),h(r)})})}document.addEventListener("DOMContentLoaded",()=>{v(),h("home")});class C extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `,document.querySelector("header>h1").textContent="Tarot de Thoth"}}customElements.define("home-screen",C);const b="modulepreload",S=function(o){return"/"+o},p={},T=function(e,t,a){let r=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const n=document.querySelector("meta[property=csp-nonce]"),i=(n==null?void 0:n.nonce)||(n==null?void 0:n.getAttribute("nonce"));r=Promise.allSettled(t.map(c=>{if(c=S(c),c in p)return;p[c]=!0;const l=c.endsWith(".css"),u=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${u}`))return;const d=document.createElement("link");if(d.rel=l?"stylesheet":b,l||(d.as="script"),d.crossOrigin="",d.href=c,i&&d.setAttribute("nonce",i),document.head.appendChild(d),l)return new Promise((f,y)=>{d.addEventListener("load",f),d.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${c}`)))})}))}function s(n){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=n,window.dispatchEvent(i),!i.defaultPrevented)throw n}return r.then(n=>{for(const i of n||[])i.status==="rejected"&&s(i.reason);return e().catch(s)})};let m=null;async function g(){if(m)return m;try{const e=await(await fetch("assets/data/cards.json")).json();return m=e,e}catch(o){throw console.error("Erro ao carregar as cartas:",o),o}}T(()=>import("./TarotCard-BhMfcB2C.js"),[]);class E extends HTMLElement{async connectedCallback(){this.innerHTML=`
        ...
        `,document.querySelector("header>h1").textContent="Todas as Cartas",await this.renderCards()}async renderCards(){try{const e=await g();e.sort((a,r)=>a.order-r.order);const t=this.querySelector("#all-cards");t.innerHTML=e.map(a=>`
                    ...
                    `).join("")}catch(e){console.error("Erro ao carregar as cartas:",e)}}}customElements.define("all-cards-screen",E);function L(o){const e=JSON.parse(localStorage.getItem("tarotHistory"))||[];e.push({date:new Date().toLocaleString(),cards:o}),localStorage.setItem("tarotHistory",JSON.stringify(e))}function q(){return JSON.parse(localStorage.getItem("tarotHistory"))||[]}class H extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `,document.querySelector("header>h1").textContent="Jogo das 3 Cartas",document.getElementById("drawCards").addEventListener("click",this.drawThreeCards.bind(this))}async drawThreeCards(){try{const t=(await g()).sort(()=>.5-Math.random()).slice(0,3),a=this.querySelector("#cards"),r=["Passado","Presente","Futuro"];a.innerHTML=t.map((s,n)=>`
