(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))i(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function t(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(r){if(r.ep)return;r.ep=!0;const o=t(r);fetch(r.href,o)}})();function h(s){const e=document.getElementById("app");e.innerHTML="",s==="all-cards"?e.innerHTML="<all-cards-screen></all-cards-screen>":s==="three-card-game"?e.innerHTML="<three-card-game-screen></three-card-game-screen>":s==="history"?e.innerHTML="<history-screen></history-screen>":e.innerHTML="<home-screen></home-screen>"}function v(){document.querySelectorAll("nav a").forEach(e=>{e.addEventListener("click",t=>{const i=e.getAttribute("href");if(i&&i.endsWith(".html"))return;t.preventDefault();const r=e.getAttribute("data-page");console.log("Navegando para:",r),h(r)})})}document.addEventListener("DOMContentLoaded",()=>{v(),h("home")});class b extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `}}customElements.define("home-screen",b);const C="modulepreload",S=function(s){return"/"+s},p={},E=function(e,t,i){let r=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const n=document.querySelector("meta[property=csp-nonce]"),a=(n==null?void 0:n.nonce)||(n==null?void 0:n.getAttribute("nonce"));r=Promise.allSettled(t.map(c=>{if(c=S(c),c in p)return;p[c]=!0;const l=c.endsWith(".css"),u=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${u}`))return;const d=document.createElement("link");if(d.rel=l?"stylesheet":C,l||(d.as="script"),d.crossOrigin="",d.href=c,a&&d.setAttribute("nonce",a),document.head.appendChild(d),l)return new Promise((f,y)=>{d.addEventListener("load",f),d.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${c}`)))})}))}function o(n){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=n,window.dispatchEvent(a),!a.defaultPrevented)throw n}return r.then(n=>{for(const a of n||[])a.status==="rejected"&&o(a.reason);return e().catch(o)})};let m=null;async function g(){if(m)return m;try{const e=await(await fetch("assets/data/cards.json")).json();return m=e,e}catch(s){throw console.error("Erro ao carregar as cartas:",s),s}}E(()=>import("./TarotCard-BhMfcB2C.js"),[]);class T extends HTMLElement{async connectedCallback(){this.innerHTML=`
            <div class="all-cards-screen">
                <h1>Todas as Cartas</h1>
                <div class="card-container" id="all-cards"></div>
                <button onclick="navigateTo('home')">Voltar</button>
            </div>
        `,await this.renderCards()}async renderCards(){try{const e=await g();e.sort((i,r)=>i.order-r.order);const t=this.querySelector("#all-cards");t.innerHTML=e.map(i=>`
                        <tarot-card data-card='${JSON.stringify(i)}'></tarot-card>
                    `).join("")}catch(e){console.error("Erro ao carregar as cartas:",e)}}}customElements.define("all-cards-screen",T);function L(s){const e=JSON.parse(localStorage.getItem("tarotHistory"))||[];e.push({date:new Date().toLocaleString(),cards:s}),localStorage.setItem("tarotHistory",JSON.stringify(e))}function q(){return JSON.parse(localStorage.getItem("tarotHistory"))||[]}class H extends HTMLElement{connectedCallback(){this.innerHTML=`
        ...
        `,document.getElementById("drawCards").addEventListener("click",this.drawThreeCards.bind(this))}async drawThreeCards(){try{const t=(await g()).sort(()=>.5-Math.random()).slice(0,3),i=this.querySelector("#cards"),r=["Passado","Presente","Futuro"];i.innerHTML=t.map((o,n)=>`
                    <div  class="card-three">
                        <h3>${r[n]}</h3>
                        <tarot-card data-card='${JSON.stringify(o)}' show-back></tarot-card>
                    </div>
